            ...
            const handleIncomingMessage = (data) => {
                ...
                setViewChat((prevViewChat) => {
                    if (!prevViewChat) {
                        ...
                        // Mostrar notificación toast para mensajes nuevos cuando el chat no está abierto
                        if (data.body.message && data.body.message.user && data.body.message.user._id !== userInfo._id) {
                            // Buscar el chat en la lista de chats para obtener el nombre
                            const chat = [...inboxList, ...archivedChats].find(c => c._id === data.body.chatId);
                            const chatName = chat ? 
                                (chat.isPrivate ? 
                                    chat.members.find(m => m.user._id !== userInfo._id)?.user?.profile?.name || 'Usuario' 
                                    : chat.label || 'Grupo') 
                                : 'Chat';
                            ...
                            toast.info(
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600 truncate">{data.body.message.text || 'Nuevo mensaje'}</p>
                                    </div>
                                </div>, 
                                {
                                    position: "top-right",
                    ...
                    // Si el mensaje es para otro chat que no está abierto, mostrar notificación
                    if (data.body.message && data.body.message.user && data.body.message.user._id !== userInfo._id) {
                        // Buscar el chat en la lista de chats para obtener el nombre
                        const chat = [...inboxList, ...archivedChats].find(c => c._id === data.body.chatId);
                        const chatName = chat ? 
    ...
    return (<>
        ...
        <div className="internal-chat-container" style={{height:'calc(100% - 200px)'}}>
            ...
            <div className="internal-chat-list px-5">
                <div className="flex justify-between items-center mb-1 p-3 bg-gray-50 dark:bg-zinc-800 rounded-lg">
